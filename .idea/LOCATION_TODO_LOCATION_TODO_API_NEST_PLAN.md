---
status: PREPARED
summary: "Location Todo의 OIDC BFF, PostgreSQL trigger pipeline, Kakao 기반 웹 UI와 멀티디바이스 알림 API 구현"
---
...
이 repo는 Location Todo의 server authority다. 계정·권한·quota, 저장 지오펜스, TODO·일정·lifecycle, 모바일 transition 판정, delayed/dwell due job, exactly-once trigger, notification outbox/inbox, FCM, desktop realtime, Kakao Local proxy, React/Vite 웹을 소유한다.
...
4. 최소 tables를 설계한다.
   ...
   - saved_geofences
   - todos, todo_schedule_windows, todo_geofences
...
2. permission policy를 한 모듈에 둔다.
   - visitor: 3 TODO, 1 saved geofence
   - user: 20 TODO, 5 saved geofence
   - superadmin: unlimited TODO, 20 saved geofence
3. create와 restore는 transaction 안에서 quota를 확인해 concurrent over-allocation을 막는다.
4. soft-deleted row는 quota에서 제외하고 inactive row는 포함한다.
...
### 6. TODO, schedule, relation, lifecycle APIs

1. content, timezone, trigger discriminated union, active/lifecycle 상태를 validation한다.
2. schedule windows를 여러 개 저장한다.
   - optional local date
   - start < end
   - end-only `24:00`
   - no cross-midnight single interval
3. one-or-more saved geofence OR relation을 account ownership과 함께 검증한다.
4. create/edit/list/get/soft-delete/restore/active toggle/complete/reactivate를 구현한다.
5. deactivate/delete/complete는 monitoring projection과 pending jobs를 transactionally cancel한다.
6. restore는 inactive로, reactivate는 clean detection cycle로 만든다.
7. optimistic concurrency/version conflict를 구조화한다.

### 7. Transition batch and state machine
...
7. inactive/completed/deleted/triggered canonical state는 late offline event보다 우선한다.
8. offline batch의 enter/exit 순서를 먼저 적용한 뒤 dwell evidence를 판정한다.

### 8. Immediate, delayed, dwell authority

1. `ENTRY_IMMEDIATE` valid entry가 trigger guard를 획득하도록 한다.
2. `ENTRY_DELAYED`는 최초 valid entry 하나만 due job을 생성하고 exit로 취소하지 않는다.
...
5. trigger transaction은 TODO 상태 전환, trigger event, inbox, outbox를 함께 commit한다.
6. simultaneous devices/geofences/jobs가 하나의 trigger만 만들도록 partial unique guard를 둔다.
7. restart/crash/retry/clock boundary test를 작성한다.

### 9. Notification delivery and realtime
...
5. TODO editor는 multiple schedules, multi-geofence OR, three trigger modes, active toggle을 제공한다.
6. TODO list는 lifecycle, next condition, geofences, monitoring state, trigger/complete/reactivate를 스캔 가능하게 표시한다.
7. notification inbox와 device/session management를 구현한다.
8. keyboard, focus, validation, mobile responsive behavior를 테스트한다.
9. API error codes를 사용자 행동 가능한 상태로 매핑하고 secret/server detail을 표시하지 않는다.
...
### 12. Documentation and deploy readiness
...
- geofence/TODO/schedule DTOs and quota error shape
...
- visitor/user/superadmin quota가 create/restore concurrency에서도 정확하다.
- web/native auth는 client secret을 노출하지 않고 per-device session/refresh family를 유지한다.
- create/reactivate inside 상태, schedule windows, multi-geofence OR, inactive/late event 규칙이 tests로 고정된다.
- immediate/delayed/dwell의 concurrent candidates가 trigger/outbox 하나만 만든다.
...
- React/Vite UI가 full geofence/TODO lifecycle과 permission request를 수행한다.
